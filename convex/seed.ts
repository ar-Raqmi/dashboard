...
// Mutation to wipe existing users and seed a fresh admin without sample data
export const resetAdmin = mutation({
  ...
  handler: async (ctx, { username, passwordHash, salt }) => {
    // 1. Delete all existing users and their related data to start fresh
    const users = await ctx.db.query("users").collect();
    for (const user of users) {
      // Delete user settings
      const settings = await ctx.db.query("userSettings").withIndex("by_user", q => q.eq("userId", user._id)).collect();
      for (const s of settings) await ctx.db.delete(s._id);

      // Delete widgets
      const widgets = await ctx.db.query("dashboardWidgets").withIndex("by_user", q => q.eq("userId", user._id)).collect();
      for (const w of widgets) await ctx.db.delete(w._id);

      // Delete layouts
      const layouts = await ctx.db.query("dashboardLayouts").withIndex("by_user", q => q.eq("userId", user._id)).collect();
      for (const l of layouts) await ctx.db.delete(l._id);

      // Delete clocks
      const clocks = await ctx.db.query("clocks").withIndex("by_user", q => q.eq("userId", user._id)).collect();
      for (const c of clocks) await ctx.db.delete(c._id);
      ...
      for (const g of goals) {
        const milestones = await ctx.db.query("milestones").withIndex("by_goal", q => q.eq("goalId", g._id)).collect();
        for (const m of milestones) await ctx.db.delete(m._id);
        await ctx.db.delete(g._id);
      }

      // Delete notes
      const notes = await ctx.db.query("notes").withIndex("by_user", q => q.eq("userId", user._id)).collect();
      for (const n of notes) await ctx.db.delete(n._id);

      // Delete events
      const events = await ctx.db.query("events").withIndex("by_user", q => q.eq("userId", user._id)).collect();
      for (const e of events) await ctx.db.delete(e._id);

      // Delete files
      const files = await ctx.db.query("files").withIndex("by_user", q => q.eq("userId", user._id)).collect();
    ...
    // 3. Create minimal necessary configurations (Settings, Widgets, Layouts, Clock)
